...
using Serilog;
using InvalidRequestException = GeeksBank.Microservice.Core.Api.SeedWork.InvalidRequestException;

namespace GeeksBank.Microservice.Core.Api.Infrastructure.Extensions
{
    /// <summary>
    ...
    public static class ExceptionMiddlewareExtensions
    {
        ...
        private static ErrorResponse GetErrorDetails(IExceptionHandlerFeature contextFeature, string appName)
        {
            ...
            return contextFeature.Error switch
            {
                ...
                SeedWork.InvalidRequestException invalidRequestException => new ErrorResponse
                {
                ...
                // Controlled server exception
                InvalidOperationException exception => new ErrorResponse
                {
                    Status = StatusCodes.Status404NotFound,
                    Service = appName.ToLower(),
                    Errors = new List<ErrorDetail>()
                ...
                // Controlled server exception
                FormatException exception => new ErrorResponse
                {
                    Status = StatusCodes.Status400BadRequest,
                    Service = appName.ToLower(),
                    Errors = new List<ErrorDetail>()
